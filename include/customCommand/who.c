  ...
  for(int i = 0 ; i < MAX_CLIENTS ; i++) {
    for(int j = 0 ; j < MAX_CLIENTS ; j++) {
      if(session[j].assignedId == i) {
        char *currentStr = malloc(512 * sizeof(char));
        ...
        snprintf(currentStr, 512,"%-8d%-20s%-20s%-20s\n", i, getNameByUserId(session[j].loggedInUserId), ipAndPort, (current -> currentId == i) ? "<- (ME)" : "");
